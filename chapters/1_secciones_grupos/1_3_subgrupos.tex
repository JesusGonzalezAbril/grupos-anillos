...
\begin{proofbox}
Veamos que $L_a(h) = ah$ es una biyección. Si $ah = bh \implies a=b$ por la propiedad cancelativa, luego es inyectiva. Si $b \in aH$ entonces, por definición, $a^{-1}b\in aH$, por tanto $L_a(a^{-1}b) = aa^{-1}b = b$, luego $L_a$ es sobreyectiva.
...
\]
También es sobreyectiva, puesto que dado $Ha \in H \backslash G$, $\phi(a^{-1}H) = Ha$.
...
El cardinal de $G/H$ (y $H \backslash G$) se llama índice de $H$ en $G$ y se denota $[G : H]$. Sabemos que $G$ es la unión disjunta de sus clases laterales por la izquierda (o por la derecha), y si $G$ es finito entonces hay exactamente $[G : H]$ clases laterales, que podemos suponer con representantes $a_i$, $i = 1,\dots, [G:H]$, luego
\[
G = \bigsqcup_{i=1}^{[G : H]} a_iH \implies |G| = \sum_{i=1}^{[G : H]} |a_iH| = [G : H]|H|.
\]
Una consecuencia inmediata de estas fórmulas es el siguiente Teorema.
