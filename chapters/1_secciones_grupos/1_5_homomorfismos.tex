...
\begin{enumerate}
    ...
    \begin{align*}
    x \in gNg^{-1} \implies x = gng^{-1}, f(n) \in H_1 \implies f(x) = f(g)f(n)f(g)^{-1} \in f(g)H_1f(g)^{-1} = H_1 \implies x \in N,
    \end{align*}
    luego $gNg^{-1} \subseteq N$, por lo que por la Proposición \ref{prop:caracterizacion_subgrupos_normales} $N$ es normal.
