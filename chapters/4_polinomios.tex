...
Pongamos $b = c(p)$.
\begin{enumerate}
    \item Si $a \mid p$ en $D[X]$ entonces $\forall i\geq 0,\ a \mid p_i$. Por tanto, por definición de $\mcd$, $b = ax \implies a \mid b$ en $D$. Por el contrario, si $a \mid b$ entonces $a$ es un divisor común de todos los $p_i$, por lo que divide a $p$.
    ...
    \[
    \operatorname{mcd}(ap_0, ap_1, \ldots, ap_n) = a \operatorname{mcd}(p_0, p_1, \ldots, p_n).
    \]
    En efecto, si $d = \operatorname{mcd}(p_0, p_1, \ldots, p_n)$ entonces dado $i\geq 0$ sabemos que $d \mid p_i$, por lo que $ad \mid ap_i$ y $ad$ es divisor común de los $ap_i$. Si
    \[
    c = \operatorname{mcd}(ap_0, ap_1, \ldots, ap_n)
    \]
    entonces $ad \mid c$. Además, como $d$ es el $mcd$ de los $p_i$ existen $a_i$ tales que
    \[
    d = a_1 p_1 + \dots + a_n p_n \implies ad = aa_1 p_1 + \dots + aa_n p_n 
    \]
    y como $c \mid a p_i \implies c \mid a a_i p_i$, por tanto
    \[
    c \mid \sum_i a a_i p_i = ad
    \]
    y finalmente $c = ad$ como queríamos ver.
